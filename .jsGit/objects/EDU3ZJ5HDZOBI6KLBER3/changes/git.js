...
function commit(message) {
  ...
  const prevObjectJsonStr = fs.readFileSync(
    `.jsGit/objects/${cache.objectID}/object.json`,
    { encoding: "utf-8" }
  );
  const prevObjectJson = JSON.parse(prevObjectJsonStr);
  ...
  allFiles.forEach((filePath) => {
    ...
    const prevFileData = FileDataOfPrevObj[filePath];
    if (prevFileData) {
...
if(cmd == "init") init();
if(cmd === "commit") commit(process.argv[3] || "no message");
